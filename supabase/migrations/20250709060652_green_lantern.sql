/*
  # Fix user signup and policy conflicts

  1. Purpose
     - Fix the "Database error saving new user" issue during signup
     - Ensure profile creation is idempotent and reliable
     - Resolve policy conflicts by properly dropping and recreating policies

  2. Changes
     - Create a robust handle_new_user function with proper error handling
     - Add ON CONFLICT DO NOTHING for idempotent profile creation
     - Fix policy conflicts by explicitly dropping before creating
     - Add utility function to fix existing users without profiles
*/

-- Drop the problematic policy to avoid conflicts
...
EXCEPTION WHEN OTHERS THEN
  ...
  RETURN NEW; -- Important: still return NEW even on error to prevent signup failure
END;
$$;
