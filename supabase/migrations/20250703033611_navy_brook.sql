/*
  ...
  1. New Tables
    ...
    - `manifestations` - User journal transformations
...
-- Function to handle user creation (create or replace to avoid conflicts)
...
BEGIN
    ...
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'manifestations') LOOP
        EXECUTE 'DROP POLICY IF EXISTS ' || quote_ident(r.policyname) || ' ON manifestations';
    END LOOP;
    
    -- Drop all policies on challenges
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'challenges') LOOP
        EXECUTE 'DROP POLICY IF EXISTS ' || quote_ident(r.policyname) || ' ON challenges';
    END LOOP;
    ...
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'challenge_progress') LOOP
        EXECUTE 'DROP POLICY IF EXISTS ' || quote_ident(r.policyname) || ' ON challenge_progress';
    END LOOP;
    
    -- Drop all policies on notification_schedules
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'notification_schedules') LOOP
        EXECUTE 'DROP POLICY IF EXISTS ' || quote_ident(r.policyname) || ' ON notification_schedules';
    END LOOP;
END $$;
...
-- Insert default challenges (using ON CONFLICT to handle duplicates)
